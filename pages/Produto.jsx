...
function ProdutoEscolhido() {
    ...
    const [carrinhoAntigo, setCarrinhoAntigo] = useState([])
    let encontrado = false

    const carrinhoCompleto = localStorage.getItem('carrinhoProdutos')
    ...
    const adicionarCarrinho = (carrinhoAtual) => {
        carrinhoAtual.push(informacoesPlanta)
        localStorage.setItem('carrinhoProdutos', JSON.stringify(carrinhoAtual))
        navigate('/carrinho', {
            state: {
                idPlanta: plantaID
            }
    ...
    const navegarCarrinho = () => {
        let novoItem = true
        const carrinhoAtual = JSON.parse(localStorage.getItem('carrinhoProdutos')) || []
